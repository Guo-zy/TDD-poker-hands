...
public class Demo {
    ...
    private void threeOfAKindCalculate(List<Integer> blackPokerHands, List<Integer> whitePokerHands) {
        ...
        for (int i = 0; i < COUNT - 3; i++) {
            ...
            if (whitePokerHands.get(i).equals(whitePokerHands.get(i + 1))
                ...
                whiteCards.setScore(score);
                whiteCards.setType("with three of a kind of " + pokerHandsSituation.get(blackPokerHands.get(i)));
    ...
    private void straightCalculate(List<Integer> blackPokerHands, List<Integer> whitePokerHands) {
        ...
        int blackSum = blackFirst + blackSecond + blackThird + blackForth + blackFifth;
        int whiteSum = whiteFirst + whiteSecond + whiteThird + whiteForth + whiteFifth;
    ...
    private void flushCalculate(List<String> blackPokerColors, List<String> whitePokerColors) {
        ...
        if (blackHset.size() == 1) {
            if (blackCards.getScore() == 5) blackCards.setStraightAndFlush(true);
            blackCards.setScore(score);
        ...
        if (whiteHset.size() == 1) {
            if (whiteCards.getScore() == 5) whiteCards.setStraightAndFlush(true);
            whiteCards.setScore(score);
    ...
    private void fullHouseCalculate(List<Integer> blackPokerHands, List<Integer> whitePokerHands) {
        ...
        if (blackFirst == blackThird && blackForth == blackFifth) {
            blackCards.setType("with full house of " + pokerHandsSituation.get(blackFirst) + " over " + pokerHandsSituation.get(blackForth));
            blackCards.setScore(score);
        ...
        if (blackFirst == blackSecond && blackThird == blackFifth) {
            blackCards.setType("with full house of " + pokerHandsSituation.get(blackForth) + " over " + pokerHandsSituation.get(blackFirst));
            blackCards.setScore(score);
        ...
        if (whiteFirst == whiteThird && whiteForth == whiteFifth) {
            whiteCards.setType("with full house of " + pokerHandsSituation.get(whiteFirst) + " over " + pokerHandsSituation.get(whiteFifth));
            whiteCards.setScore(score);
        ...
        if (whiteFirst == whiteSecond && whiteThird == whiteFifth) {
            whiteCards.setType("with full house of " + pokerHandsSituation.get(whiteThird) + " over " + pokerHandsSituation.get(whiteFirst));
            whiteCards.setScore(score);
    ...
    private void straightAndFlushCalculate() {
        ...
        if (blackCards.isStraightAndFlush()) {
            blackCards.setScore(score);
            blackCards.setType("straight flush of " + blackCards.getParticularNums());
        }
        if (whiteCards.isStraightAndFlush()) {
            whiteCards.setScore(score);
            whiteCards.setType("straight flush of " + whiteCards.getParticularNums());
        }
    }
